...
export default function ProductDetailPage() {
  ...
  const [editingVersion, setEditingVersion] = useState(null); // null = buat versi baru, { id, versionNumber, isActive } = edit versi ini
  ...
  // ─── RECIPE HANDLERS ─────────────────────────────────────────────────────
  const openRecipeEditor = (variant = null, versionToEdit = null) => {
    ...
    if (versionToEdit && versionToEdit.ingredients?.length > 0) {
      // Edit spesifik versi yang dipilih oleh user
      setRecipeIngredients(
        versionToEdit.ingredients.map((ing) => ({
          inventoryItemId: ing.inventoryItemId,
          quantity: ing.quantity,
        }))
      );
    } else {
      // Mode buat formulasi versi baru
      const targetRecipe = variant ? variant.recipe : product.recipe;
      const baseVersion = targetRecipe?.activeVersion || (targetRecipe?.versions?.[0] || null);

      if (baseVersion && baseVersion.ingredients?.length > 0) {
        setRecipeIngredients(
          baseVersion.ingredients.map((ing) => ({
            inventoryItemId: ing.inventoryItemId,
            quantity: ing.quantity,
          }))
        );
      } else if (variant && product.recipe?.activeVersion?.ingredients?.length > 0) {
  ...
  const handleSaveRecipe = (e) => {
    ...
    startTransition(async () => {
      if (editingVersion) {
        // Mode edit versi existing
        const toastId = toast.loading(`Menyimpan perubahan Versi ${editingVersion.versionNumber}...`);
        ...
        toast.success(`Formulasi Versi ${editingVersion.versionNumber} berhasil diperbarui!`, { id: toastId });
        setIsEditingRecipe(false);
        setEditingVersion(null);
        loadData();
      } else {
        // Mode buat versi baru
  ...
  const handleSetActiveVersion = (ver) => {
    startTransition(async () => {
      const toastId = toast.loading(`Mengaktifkan formulasi Versi ${ver.versionNumber}...`);
      const res = await setActiveRecipeVersion({ versionId: ver.id });
      if (res?.error) {
        toast.error(res.error, { id: toastId });
        return;
      }
      toast.success(`Versi ${ver.versionNumber} kini menjadi formulasi aktif!`, { id: toastId });
      loadData();
    });
  };
  ...
  const allVersions = activeRecipe?.versions || (activeRecipe?.activeVersion ? [activeRecipe.activeVersion] : []);
        ...
        <div className="space-y-6">
          ...
          {!isEditingRecipe ? (
            <div className="space-y-6">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-base font-bold text-slate-900">
                      Formulasi Resep:{' '}
                      {recipeTargetVariant
                        ? `${product.name} — Varian: ${recipeTargetVariant.name}`
                        : product.name}
                    </h2>
                    <span className="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200 text-[11px] font-mono font-bold">
                      {allVersions.length} Versi Dibuat
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 mt-1">
                    Berikut seluruh versi formulasi bahan baku yang pernah dibuat. Bahan dari versi yang berstatus &quot;Aktif Digunakan&quot; otomatis dipotong saat pesanan dibayar di kasir.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {recipeTargetVariant && (
                    <button
                      ...
                      className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold transition-all"
                    >
                      &larr; Lihat Resep Induk
                    </button>
                  )}
                  <button
                    onClick={() => openRecipeEditor(recipeTargetVariant, null)}
              ...
              {/* Daftar Semua Versi Resep yang Pernah Dibuat */}
              {allVersions.length > 0 ? (
                <div className="space-y-5">
                  {allVersions.map((ver) => {
                    ...
                    const verMargin =
                      targetPrice > 0
                        ? (((targetPrice - ver.estimatedHpp) / targetPrice) * 100).toFixed(1)
                        : 0;
                    ...
                    return (
                      <div
                        key={ver.id}
                        className={cn(
                          'bg-white rounded-2xl border transition-all shadow-xs overflow-hidden',
                          isCurrentActive
                      ...
                      >
                        ...
                        <div
                          className={cn(
                            'p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b',
                            isCurrentActive
                        ...
                        >
                          <div className="flex flex-wrap items-center gap-2.5">
                            <span
                              className={cn(
                                'px-3 py-1 rounded-xl text-xs font-mono font-bold border',
                                isCurrentActive
                            ...
                            {isCurrentActive ? (
                              <span className="px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 text-[11px] font-bold flex items-center gap-1.5 shadow-2xs">
                                <span className="w-2 h-2 rounded-full bg-emerald-600 animate-pulse"></span>
                                Aktif Digunakan (Kasir/POS)
                              </span>
                            ) : (
                              <span className="px-2.5 py-1 rounded-full bg-slate-200/80 text-slate-600 text-[11px] font-semibold">
                                Arsip Historis
                              </span>
                            )}

                            <span className="text-xs text-slate-400 font-mono">
                              &bull; {formatDateTime(ver.createdAt)}
                            </span>
                          </div>

                          {/* Metrik HPP & Tombol Aksi Edit & Jadikan Aktif */}
                          <div className="flex flex-wrap items-center gap-3">
                            <div className="text-right font-mono pr-2 border-r border-slate-200 hidden sm:block">
                              <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">
                                Estimasi HPP / Margin
                              </span>
                              <span className="text-xs font-extrabold text-emerald-700">
                                {formatRupiah(ver.estimatedHpp)}
                              </span>
                            ...
                            {/* Tombol Edit Versi Ini */}
                            <button
                              type="button"
                              onClick={() => openRecipeEditor(recipeTargetVariant, ver)}
                              className="px-3.5 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold shadow-xs transition-all flex items-center gap-1.5"
                              title={`Edit bahan baku & takaran pada Versi ${ver.versionNumber}`}
                            >
                              <svg
                            ...
                            {!isCurrentActive && (
                              <button
                                ...
                                className="px-3.5 py-2 bg-white hover:bg-emerald-50 text-emerald-700 rounded-xl text-xs font-bold border border-emerald-300 shadow-2xs transition-all flex items-center gap-1.5"
                                title="Jadikan versi ini sebagai formulasi aktif kasir"
                              >
                                <svg
                                  className="w-3.5 h-3.5"
                                  fill="none"
                        ...
                        {/* Tabel Bahan Baku Versi Ini */}
                        <div className="overflow-x-auto">
                          <table className="w-full text-left text-xs text-slate-700">
                            <thead className="bg-slate-50/75 text-slate-500 border-b border-slate-100 uppercase tracking-wider text-[10px]">
                              <tr>
                                <th className="py-2.5 px-4 font-semibold">Bahan Baku (Inventaris)</th>
                                <th className="py-2.5 px-4 font-semibold">Takaran per Porsi</th>
                                <th className="py-2.5 px-4 font-semibold">Biaya Rata-rata (WAC)</th>
                                <th className="py-2.5 px-4 font-semibold text-right">Subtotal HPP</th>
                              </tr>
                            </thead>
                            ...
                            <tfoot className="border-t border-slate-200 bg-slate-50/60">
                              <tr>
                                <td
                                  colSpan="3"
                                  className="py-2.5 px-4 font-bold text-slate-700 text-right uppercase text-[10px]"
                                >
                                  Total Estimasi HPP Versi {ver.versionNumber}:
                                </td>
                                <td className="py-2.5 px-4 text-right font-mono font-extrabold text-emerald-700 text-xs">
                                  {formatRupiah(ver.estimatedHpp)}
                                </td>
                              </tr>
                            </tfoot>
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-12 text-center text-slate-400 text-xs shadow-xs">
                  <div className="max-w-xs mx-auto space-y-3">
                    <p>Menu ini belum memiliki formulasi resep.</p>
                    <button
                      onClick={() => openRecipeEditor(recipeTargetVariant, null)}
                      className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-xs transition-all inline-flex items-center gap-1.5"
                    >
                      + Susun Resep Pertama
                    </button>
                  </div>
            ...
            <form onSubmit={handleSaveRecipe} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                <div>
                  <h2 className="text-base font-bold text-slate-900">
                    {editingVersion
                      ? `Edit Formulasi Versi ${editingVersion.versionNumber}: ${recipeTargetVariant ? `${product.name} (${recipeTargetVariant.name})` : product.name}`
                      : `Formulasi Versi Resep Baru: ${recipeTargetVariant ? `${product.name} (${recipeTargetVariant.name})` : product.name}`}
                  </h2>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {editingVersion
                      ? `Perubahan takaran atau bahan akan langsung disimpan ke Versi ${editingVersion.versionNumber}.`
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-bold">
                        Mode Edit Versi {editingVersion.versionNumber} ({editingVersion.isActive ? 'Sedang Aktif Digunakan' : 'Arsip Historis'})
                      </p>
                      <p className="text-[11px] text-amber-800 mt-0.5">
                        Anda dapat mengubah, menambah, atau menghapus bahan baku pada versi ini jika terdapat kesalahan input data.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                {recipeTargetVariant && (
                  <div className="p-3.5 rounded-2xl bg-blue-50/80 border border-blue-200 text-blue-900 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs">
                    ...
                    {product.recipe?.activeVersion?.ingredients?.length > 0 && !editingVersion && (
                      <button
                        type="button"
                        onClick={() => {
                          setRecipeIngredients(
              ...
              <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row items-center justify-between gap-4 font-mono">
                <div>
                  <p className="text-xs text-emerald-900">
                    Estimasi HPP Resep: <strong className="text-emerald-700 font-extrabold text-sm">{formatRupiah(liveEstimatedHpp)}</strong>
                  </p>
